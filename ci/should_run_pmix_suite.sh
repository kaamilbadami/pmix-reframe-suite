    ...
    schedule)
        if [[ ! -f "$state_file" ]]; then
            ...
            if ! mapfile -t state_lines < "$state_file"; then
                state_valid=0
            elif (( ${#state_lines[@]} != 2 )); then
                state_valid=0
            elif [[ ${state_lines[0]} =~ ^PMIX_COMMIT=([[:xdigit:]]{40})$ ]]; then
                saved_sha=${BASH_REMATCH[1]}
            else
                state_valid=0
            fi

            if (( state_valid )); then
                if [[ ${state_lines[1]} =~ ^LAST_SUCCESS_EPOCH=(0|[1-9][0-9]{0,18})$ ]]; then
            ...
            if (( ! state_valid )); then
            ...
            elif [[ "$saved_sha" != "$current_sha" ]]; then
                ...
                if (( saved_epoch_decimal > now_epoch )); then
                    ...
                    if (( state_age >= 86400 )); then
                        ...
                        run_suite=0
                        run_reason="OpenPMIx master is unchanged and the last successful complete run is ${state_age} seconds old"
                    fi
                fi
            fi
        fi
